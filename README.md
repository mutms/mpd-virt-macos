...
drives **Parallels Desktop Pro** to create and manage `mpd-machine` VMs.
...
```
mpd-virt create  <octet>      Clone the template into mpd-machine-<octet>,
                              provision it, write SSH config block, import
                              the WireGuard tunnel into WireGuard.app.
mpd-virt delete  <octet>      Delete mpd-machine-<octet>. Removes the per-VM
                              bookkeeping under ~/.mpd-virt/<octet>/ and the
                              VM's SSH config entries; preserves
                              ~/.mpd-virt/conf/ (CA + WG identity persist).
mpd-virt start   <octet>      prlctl start mpd-machine-<octet>.
mpd-virt stop    <octet>      prlctl suspend mpd-machine-<octet>.
...
network and is also the suffix in the VM name (`mpd-machine-<octet>`) and
...
1. Verify prerequisites: Parallels Desktop Pro installed, `prlctl` on PATH,
   a Parallels VM template named `mpd-machine-template`, host tooling
   (`ssh`, `ssh-keygen`, `scp`, `security`, `route`).
2. SSH key: use `~/.ssh/id_ed25519.pub` if present; offer to generate it
   otherwise.
3. Refuse if Parallels already has a VM named `mpd-machine-<octet>`.
4. Generate / reuse persistent identity in `~/.mpd-virt/conf/`:
   ...
   - Per-VM WG keypair + configs (`wireguard/machine/<octet>/…`) —
...
6. Clone `mpd-machine-template` as `mpd-machine-<octet>`. Boot, wait for
   SSH.
7. Push CA + WG server.conf into the VM as
   `~/.mpd/conf/{caroot/rootCA.pem, wireguard/mpd0.conf}`. Write
   `~/.mpd/conf/platform.env` (`MPD_PLATFORM=machine`, …).
...
11. Add `~/.ssh/config` entries:
    - `Host mpd-machine-<octet>` → Parallels Shared IP.
    - `Host mpd-machine-<octet>-{php,node,util}` → `ProxyJump
      mpd-machine-<octet>`.
...
│   ├── wireguard/
│   │   ├── mac.{private,public}
│   │   └── machine/<octet>/{private,public,server.conf,client.conf}
│   └── service/
└── <octet>/env                    ← per-VM bookkeeping (UUID, IP, …)
```

`~/.mpd/` is **not** created on the host — that path is exclusively the
in-VM runtime state directory inside each mpd-machine VM.
